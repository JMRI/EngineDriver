...
class WebSocketRunnable implements Runnable {
  ...
  private boolean closeRequested = false;  //used to avoid error message when shutdown is intentional
...
//        Log.d(Consts.APP_NAME, "starting WebSocketRunnable.run()");
    ...
    if (jmriWebSocket != null) {
      Log.d(Consts.APP_NAME, "calling webSocket.disconnect()");
      ...
      switch (msg.what) {
        case MessageType.SHUTDOWN:
          ...
          try {  //tell the server goodbye
            closeRequested = true;  //make note that this disconnect was requested
...
//          break;
  ...
  private void locoRequested(String fragmentName, String rosterName, String rosterAddress, int locoDirection) {
    ...
    } else if (t.isConfirmed()) {  //already exists, let client know (since jmri won't respond twice)
      t.addFragment(fragmentName);  //include this fragment in the where-used list
...
//                    Log.d(Consts.APP_NAME, "JmriWebSocket, goodbye message received and looper ended");
    ...
    private void receivedRouteList(JSONArray msgJsonArray) throws JSONException {
      ...
      for (int i = 0; i < msgJsonArray.length(); i++) {
        ...
        if (!data.getString("userName").equals("null")) {  //skip any without a username TODO:verify we still want to handle this this way
          ...
          String s = r.getChangeStateJson(Consts.STATE_UNKNOWN);  //send no-change request to start listener
          this.webSocketConnection.sendTextMessage(s);
        }
      }
...
//                            ", value=" + data.getString("value"));
    ...
    private void receivedHello(JSONObject data) throws JSONException {
      ...
      //inform everyone about the change in state
      mainApp.sendMsg(permaFragment.permaFragHandler, MessageType.CONNECTED);
      //put other values into shared variables for later use (Note: they have their own xx_CHANGED msgs)
...
//          Log.d(Consts.APP_NAME, "key=" + key + ", val=" + val);
    ...
    public void onClose(int code, String closeReason) {
      ...
      if (!closeRequested) {  //tell the user, but only if not expected
        mainApp.sendMsg(permaFragment.permaFragHandler, MessageType.MESSAGE_LONG, s);
      }
      mainApp.sendMsg(permaFragment.permaFragHandler, MessageType.DISCONNECTED); //tell the app
    }
