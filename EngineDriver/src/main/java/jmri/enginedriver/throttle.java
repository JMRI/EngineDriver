...
public class throttle extends AppCompatActivity implements android.gesture.GestureOverlayView.OnGestureListener, PermissionsHelper.PermissionsHelperGrantedCallback {
    ...
    private static final int SOUNDS_TYPE_LOCO_START_UP = 3;
    private static final int SOUNDS_TYPE_LOCO_SHUTDOWN = 4;
...
//                    Log.d("Engine_Driver", "SoundScheduleNextSoundToPlay.run: (locoSound) wt: " + whichThrottle + " snd: " + mSound);
                ...
                case SOUNDS_TYPE_BELL: // bell
                    ...
                    break;
//                case SOUNDS_TYPE_LOCO_START_UP:
//                case SOUNDS_TYPE_LOCO_SHUTDOWN:
//                    break;
            }
        }
    }
        ...
        for (int i = 0; i <= 1; i++) {
            if (mainapp.prefDeviceSounds[i].toLowerCase(Locale.ROOT).contains(".ipls")) {
                ...
                if (!mainapp.iplsFileName.equals("")) {
                    ...
                    loadSoundFromFile(SOUNDS_TYPE_LOCO, i, SOUNDS_STARTUP_INDEX, this, mainapp.iplsLocoSoundsFileName[SOUNDS_STARTUP_INDEX]);
                    loadSoundFromFile(SOUNDS_TYPE_LOCO, i, SOUNDS_SHUTDOWN_INDEX, this, mainapp.iplsLocoSoundsFileName[SOUNDS_SHUTDOWN_INDEX]);
...
//        Log.d("Engine_Driver", "soundsStopAllSoundsForLoco : (locoSound) wt: " + whichThrottle);
    ...
    void soundsShowHideAllMuteButtons() {
        for (int i = 0; i < threaded_application.SOUND_MAX_SUPPORTED_THROTTLES; i++) {
            soundsShowHideMuteButton(i);
        }
    }

    void soundsShowHideMuteButton(int whichThrottle) {
    ...
    void soundsEnableDisableMuteButton(int whichThrottle, boolean newEnabledState) {
        if ( (whichThrottle<mainapp.maxThrottlesCurrentScreen) && (whichThrottle<threaded_application.SOUND_MAX_SUPPORTED_THROTTLES) ) {
            if (bMutes!=null) {
                if (bMutes[whichThrottle]!=null) {
